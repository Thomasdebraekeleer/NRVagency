...
export function FooterGroup({
  ...
  return (
    <div
      className={cn(
        "flex grow flex-col items-center justify-center pb-6 text-[clamp(20px,_1vw_+_14px,_24px)] text-[#f1f1e7] md:block md:grow-0 md:py-6",
        className,
      )}
    >
        ...
        {links.map((link, index) => (
          <li key={index} className="">
            {isMagnetic ? (
              <Magentic
                strength={50}
                className="relative text-[0.8em] text-[#f1f1e7]"
                scrambleParams={{
                  text: link.text,
                  chars: "-xx",
                }}
              ...
              <p className="text-[0.8em] text-[#f1f1e7]">{link.text}</p>
            )}
          </li>
        ))}
      </ul>
