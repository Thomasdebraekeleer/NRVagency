...
export function HeroLogo() {
  ...
  useEffect(() => {
    // Animation de rotation du logo au hover
    if (logoRef.current) {
      ...
      // Ajouter des écouteurs directement sur le SVG en plus
      const svg = logoRef.current;
      const parent = svg.closest('a');
      
      const handleEnter = () => {
        if (logoAnimationTl.current) {
  ...
  const handleMouseEnter = () => {
    if (logoAnimationTl.current && logoRef.current) {
      logoAnimationTl.current.play();
    }
  };
  ...
  return (
    <div className="absolute top-8 md:top-12 left-6 md:left-8 z-[100]">
      <Magentic
        href={links.home}
        strength={50}
